...
from utils.Adam import Adam_GCC, AdamW, AdamW_GCC
...
def make_optimizer(args, model, center_criterion):
    ...
    if args.optimizer == 'SGD':
        ...
        optimizer_function = SGDW
        kwargs = {
            'momentum': args.momentum,
            'dampening': args.dampening,
            'nesterov': args.nesterov
            }
    elif args.optimizer == 'SGDW_GCC':
        optimizer_function = SGDW_GCC
        kwargs = {
            'momentum': args.momentum,
            'dampening': args.dampening,
            'nesterov': args.nesterov
    ...
    elif args.optimizer == 'ADAM_GCC':
        optimizer_function = Adam_GCC
        kwargs = {
            'betas': (args.beta1, args.beta2),
            'eps': args.epsilon,
            'amsgrad': args.amsgrad
    ...
    elif args.optimizer == 'ADAMW_GCC':
        optimizer_function = AdamW_GCC
        kwargs = {
            'betas': (args.beta1, args.beta2),
            'eps': args.epsilon,
            'amsgrad': args.amsgrad
    ...
    #adding by syl on 2020.07.21
    elif args.optimizer == 'Ranger':
        optimizer_function = Ranger
        kwargs = {}
    else:
        raise Exception()
    ...
    if args.center == 'yes':
        optimizer_center = torch.optim.SGD(center_criterion.parameters(), lr=args.center_lr) if not args.test_only else None
        return optimizer_function(trainable, **kwargs), optimizer_center
    else:
